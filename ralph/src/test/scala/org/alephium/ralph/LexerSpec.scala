...
import org.oxygenium.protocol.{OXM, Hash, PublicKey}
...
abstract class LexerSpec(fileURI: Option[java.net.URI]) extends OxygeniumSpec {
  ...
  it should "parse lexer" in {
    ...
    parsePositioned("1alph", Lexer.typedNum(_)).get.value.v is Val.U256(OXM.oneOxm)
    parsePositioned("1 alph", Lexer.typedNum(_)).get.value.v is Val.U256(OXM.oneOxm)
